...
Пример:
 ...
 3. В контроллере установлен обработчик на событие изменения этого свойства, он "слышит" событие и вызывает метод emit(), генерируя "новое событие"  =>
...
Первым подключается файл Components.js, затем контроллер(ы) и далее компоненты
...
- **emit(name, data)** - генерирует пользовательское событие. 
_name_ (строка) - имя события, обязательный аргумент, помимо назывнаия события может сообщать его тип через двоеточие
_data_ (произвольный тип) - данные, которые будут переданы в обработчик события, необязательный аргумент
- **listen(name, func, context)** - устанавливает обработчик на пользовательское событие. 
_name_ (строка) - имя события, обязательный аргумент. Можно уточнить тип события, указав его через двоеточие: "event:type"
...
Возвращает объект с методом *stopListening()*, при помощи которого можно удалить установленный обработчик. 
...
После этого необходимо создать компонент "**Button**", к которому будет привязана эта кнопка. Создаем файл Button.js, подключаем его после Components.js и после Controller.js (в дальнейшем все компоненты лучше подключать после контроллера, в некоторых случаях этот момент может быть важным). Добавляем в него следующий код:
```
...
Component.create('Button', {
    init: function () {
        ...
        this.controller.listen('change:' + prop, function (status) {
            this.status = status;
        }, this);
    }
});
...
Когда свойство меняется, контроллер генерирует событие 'change' с типом, соответствующим имени изменившегося события, указанным через двоеточие. На него можно поставить обработчик:
```
new Controller(function () {
    this.listen('change:modalWindow', function (status) {
...
Задача контроллера - сообщить другим компонентам о произошедших изменениях. Для этого можно сгенерировать собственное событие. 
 ...
 `this.emit('modalWindowChanged', status);`
...
Подключаем файл Modal.js после Controller.js (как и любые другие компоненты). И добавляем на страницу код:
...
```
init: function () {
    this.controller.listen('change:modalWindow', function (status) {
        if(status === true) {
            this.show();
        } else {
            this.hide();
